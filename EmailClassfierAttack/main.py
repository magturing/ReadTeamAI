...
def preprocess_message(message):
    ...
    # return " ".join(tokens)
    return message  # No preprocessing at all


def poison_labels(df, fraction=0.3):
    # Flip a fraction of the labels
    n = int(len(df) * fraction)
    df_poisoned = df.copy()
    indices = df_poisoned.sample(n).index
    df_poisoned.loc[indices, 'label'] = df_poisoned.loc[indices, 'label'].map({'ham': 'spam', 'spam': 'ham'})
    return df_poisoned
...
def train(dataset):
    ...
    df = pd.read_csv(dataset)
    df = poison_labels(df, fraction=0.3)  # Flip 30% of the labels
    df = preprocess_dataframe(df)
